...
import AlertMulta from "@/components/alertMulta";
...
export default function EmprestimosPage() {
  ...
  const carregarEmprestimosHistorico = async () => {
    try {
  ...
  const carregarEmprestimosByMembroHistorico = async () => {
    try {
      const response = await api.get(`/emprestimos/${input}/historico`)
      const res = response.data;
      setEmprestimos(res)
    } catch (error: any) {
      toast.error("Erro ao carregar dados dos empréstimos")
  ...
  useEffect(() => {
    if (input.length !== 8) {
      ...
      if (status === 'ativos')
        ...
        carregarEmprestimosByMembroHistorico()
    }
  ...
  const handleDevolucao = async (id: number) => {
    try {
      const resul = await api.post('/emprestimos/devolucao', {
        emprestimoId: id
      })

      const multa = resul.data.multa;
      setMulta(multa)
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <CreateEmprestimoModal onSucesso={carregarEmprestimosHistorico} />
      </div>
                    ...
                    }
                    {
                      status !== 'todos' && (
                        <TableCell className="text-right space-x-2">
                          ...
                          <AlertMulta multa={multa} emprestimoId={emp.id}>
                            <Button
                              size="sm"
